...
def getitem(v, k):
    ...
    """
    fun = v.f
    return fun[k] if k in fun.keys() else 0
...
def add(u,v):
    ...
    assert u.D == v.D
    D = u.D
    union_valued_keys = set(u.f.keys()) | set(v.f.keys())
    f = {k: u[k] + v[k] for k in union_valued_keys }
    return Vec(D, f)
...
def scalar_mul(v, alpha):
    ...
    if alpha == 0:
        ...
        f = {k:alpha*v[k] for k in v.f.keys()}
        return Vec(v.D, f)
