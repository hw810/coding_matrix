...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    return sum([a*b for a,b in zip(vote_a, vote_b)])
...
def most_similar(sen, voting_dict):
    ...
    """
    exclusive_voting_dict = voting_dict.copy()
    exclusive_voting_dict.pop(sen)
    return max(exclusive_voting_dict.keys(), key=lambda x: policy_compare(x, sen, voting_dict))
...
def least_similar(sen, voting_dict):
    ...
    exclusive_voting_dict = voting_dict.copy()
    exclusive_voting_dict.pop(sen)
    return min(exclusive_voting_dict.keys(), key=lambda x: policy_compare(x, sen, voting_dict))




## 5: (Task 2.12.5) Chafee, Santorum
f_voting = open('voting_record_dump109.txt')
str_list_voting = list(f_voting)
voting_dict = create_voting_dict(str_list_voting)
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    return 1.0 * sum([policy_compare(sen, sen_i, voting_dict) for sen_i in sen_set]) / len(sen_set)

democrat_set = [one_sen.split()[0] for one_sen in str_list_voting if one_sen.split()[1] == 'D']
all_set = voting_dict.keys()
most_average_Democrat = max(all_set, key = lambda x: find_average_similarity(x, democrat_set, voting_dict)) # give the last name (or code that computes the last name)
...
def find_average_record(sen_set, voting_dict):
    ...
    Example: 
        ...
        [0.0, 1.0, 1.0]
    """
    list_vec = [voting_dict[x] for x in sen_set]
    n_element = len(list_vec)
    avg_vec = [1.0*sum(x)/n_element for x in zip(*list_vec)]
    return avg_vec

average_Democrat_record = find_average_record(democrat_set, voting_dict) # give the vector as a list
extra_voting_dict = voting_dict.copy()
extra_voting_dict['avg_democrat'] = average_Democrat_record
most_average_Democrat_2 = most_similar('avg_democrat', extra_voting_dict)
...
def bitter_rivals(voting_dict):
    ...
    Example: 
        ...
        True
    """
    least_similar_pairs = [(x, least_similar(x, voting_dict)) for x in voting_dict]
    rival_pairs = [(x, y) for x in voting_dict for y in voting_dict if x != y]
    return min(rival_pairs, key = lambda x: policy_compare(x[0], x[1], voting_dict)) 
